###############################################################
## SOURCE: https://github.com/mkfreeman/hierarchical-models/ ##
##   Visualization: http://mfviz.com/hierarchical-models/    ##
###############################################################
...
df <- df %>% mutate(salary = bases + experience * raises)
...
ggplot(data=df, aes(x=experience, y=salary, )) +
  geom_point() + 
...
ggplot(data=df) +
  geom_line(aes(x=experience, y=random.intercpet.preds, group = department, colour = department)) + 
  geom_point(data=df, aes(x=experience, y=salary, group = department, colour = department)) +
  geom_smooth(aes(x=experience, y=salary),method="lm",se=FALSE,lty=5)+
  labs(x="Experience", y="Predicted Salary") +
  ggtitle("Varying Intercept Salary Prediction") + 
  scale_colour_discrete('Department')     
...
ggplot(data=df, aes(x=experience, y=random.slope.int.preds, group = department, colour = department)) +
  geom_point(data=df, aes(x=experience, y=salary)) + geom_line() + 
  labs(x="Experience", y="Predicted Salary") +
